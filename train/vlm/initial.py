...
class SFTCollatorRightPad:
    ...
    def __call__(self, batch):
        ...
        for ex in batch:
            ...
            if len(img_list) != 5:
                raise RuntimeError(f"[COLLATOR][FATAL] expect 5 image files, got {len(img_list)}")
    ...
    *,
    ...
    for i in range(n):
        ...
        if not isinstance(img_files, list) or len(img_files) != 5:
            raise RuntimeError(
                f"[PRECOMPUTE][FATAL] expect 5 image files, got {type(img_files)} len={len(img_files) if isinstance(img_files, list) else 'NA'}"
            )
...
def build_sft_datasets():
    ...
    if is_main_process():
        log(
            ...
            f"train_short={len(ds_short_train)}"
        )
    ...
    return ds_short_train, ds_eval_short
...
class TinyLLMTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False,**kwargs):
        """
        把多余的字段弹掉，把 vision_feats 等显式传给 forward。
        """
...
def main():
    ...
    ds_short, ds_eval_short = build_sft_datasets()
    if SFT_STAGE == 1:
    ...
    while start_step < total_steps:
        target_step = min(total_steps, start_step + VIT_CHUNK_STEPS)
        ...
        if os.path.isdir(cand):
            ...
            trainer.save_model(cand)
            trainer.save_state()
            # 你想要 base+lora 拆分，也可以顺手：
